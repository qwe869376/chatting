...
smalltalk.prompt('身份??', '你的名字?', '').then(function(name) {
...
function incomeHtml(tname,head){
		...
		html+='</div><div class="body1"><input type="text" class="input" /></div>';
		html+='<div class="foot"><a class="send" href="javascript:void(0)">?送(Enter)</a></div></div></div></div></div>';
		$('#chat').append(html);
	}